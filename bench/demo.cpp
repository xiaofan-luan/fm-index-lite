...
main() {
    ...
    auto show = [&](const char* what, const std::vector<uint64_t>& ids) {
        ...
        for (size_t i = 0; i < ids.size(); ++i)
            printf("%s%llu", i ? "," : "", (unsigned long long)ids[i]);
        printf("}\n");
    };

    printf("\n-- substring Count (case-insensitive, per-document) --\n");
    printf(
        "Count(\"error\") = %zu   Count(\"ERROR\") = %zu   Count(\"ErRoR\") = "
        "%zu\n",
        fm.Count(B("error"), 5),
        fm.Count(B("ERROR"), 5),
        fm.Count(B("ErRoR"), 5));
    ...
    show("FuzzyMatchingDocs(\"eror\", k=1)",
         fm.FuzzyMatchingDocs(B("eror"), 4, 1));
    ...
    printf(
        "  (letters are lowercased here because the index is "
        "case-insensitive)\n");
    return 0;
}
